def child_can_attack():
    return check_requirements([{'swordkokiri':1}, {'stick':1}]) #bombs too?

def has_explosives():
    return check_requirements([{'bombs':1}, {'bombchus':1}]) # to do: indicate somehow that bombchus are out of logic (different color fo check?)

def botw_access():
    return check_requirements({'storms':1})
...
def spirit_access():
    ...
    return check_requirements([{is_child:{desert_colossus_access:True}}, {is_adult:{desert_colossus_access:{'strength':2}}}])

def desert_colossus_child_access():
    return check_requirements({'ocarina':1, 'requiem':1})

def desert_colossus_access():
    return check_requirements([{'ocarina':1, 'requiem':1}, {is_adult:{'magic':1, 'lens':1, 'shot':2}}, {is_adult:{'magic':1, 'lens':1, 'bootshover':1}}])
